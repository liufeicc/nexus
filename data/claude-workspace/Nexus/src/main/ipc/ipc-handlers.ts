...
export function registerIpcHandlers(): void {
  ...
  /**
   * 保存用户偏好
   * 使用确定性 ID 确保每个会话只有一条用户偏好记录
   * 若已存在则更新，否则新建
   */
  ipcMain.handle(IPC_CHANNELS.MEMORY_SAVE_USER_PREF, (_event, content: string) => {
    try {
      const activeSession = DatabaseService.getInstance().getSessionDAO().getActive()
      if (!activeSession) {
        return { success: false, error: '无活跃会话' }
      ...
      const nexusSessionId = String(activeSession.id)
      const dao = DatabaseService.getInstance().getMemoryDAO()
      const prefId = `user-prefs-${nexusSessionId}`

      // 检查是否已存在用户偏好条目
      const existingEntries = dao.getEntries(nexusSessionId, 'user')
      const existingEntry = existingEntries.find(e => e.id === prefId)

      if (existingEntry) {
        // 已存在，更新内容
        dao.updateEntry(prefId, content, nexusSessionId)
        // 同步更新 facts 表
        try { dao.deleteFact(prefId, nexusSessionId) } catch { /* 可能不存在 */ }
        dao.insertFact(content, nexusSessionId, prefId, 1.0, 'user')
      } else {
        // 不存在，新建条目
        dao.insertEntry(nexusSessionId, content, 'user', prefId)
        // 同步写入 facts 表
        dao.insertFact(content, nexusSessionId, prefId, 1.0, 'user')
      }
   ...
   * 返回当前会话的用户偏好内容
   */
  ipcMain.handle(IPC_CHANNELS.MEMORY_GET_USER_PREF, () => {
    try {
      const activeSession = DatabaseService.getInstance().getSessionDAO().getActive()
      if (!activeSession) {
        return { success: false, error: '无活跃会话' }
      ...
      // 获取所有 scope='user' 的条目，返回第一条内容
      const userEntries = dao.getEntries(nexusSessionId, 'user')
      const content = userEntries.length > 0 ? userEntries[0].content : ''
