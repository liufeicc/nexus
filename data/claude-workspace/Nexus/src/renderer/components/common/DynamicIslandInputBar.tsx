...
export function DynamicIslandInputBar({
  ...
  return (
    <div className="island-agent-fixed-bar">
      ...
      {/* 输入行 — 用户偏好按钮 + 输入框 + 发送/停止 + 关闭 */}
      <div className="island-agent-input-row">
        {/* 用户偏好按钮 */}
        <button
          className="island-user-pref-btn"
          onClick={(e) => { e.stopPropagation(); onOpenUserPref() }}
          title={t('dynamicIsland.userPreferences')}
        >
          <svg viewBox="5 2 90 95" fill="none" stroke="currentColor" strokeWidth="8" strokeLinecap="round" strokeLinejoin="round">
            <circle cx="50" cy="30" r="24" />
            <path d="M8 97c0-23 19-40 42-40s42 17 42 40" />
          </svg>
        </button>
        <textarea
          ref={inputRef}
